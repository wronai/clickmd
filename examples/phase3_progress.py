...
PROGRESS_ITEMS_TOTAL = CONSTANT_50
PROGRESS_DOWNLOAD_STEPS = CONSTANT_30
...
def demo_progress_bar() -> None:
    ...
    clickmd.md("\n## Custom Progress Bar\n")
    with clickmd.ProgressBar(
        total=PROGRESS_DOWNLOAD_STEPS, label="Downloading", color="green"
    ) as bar:
...
def demo_spinners() -> None:
    ...
    clickmd.table(
        ...
        rows=[
            [name, " ".join(frames[:CONSTANT_6]) + "..."]
            for name, frames in list(clickmd.SPINNERS.items())[:CONSTANT_8]
        ],
    )
...
def demo_countdown():
    ...
    clickmd.md("\n# ⏱️ Countdown\n")
    clickmd.countdown(CONSTANT_3, "Demo starting in")
