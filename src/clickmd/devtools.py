...
def _format_dict(
    obj: dict, renderer: MarkdownRenderer, depth: int, max_depth: int, max_items: int, indent: str
) -> str:
    """Format a dictionary."""
    if len(obj) == 0:
...
def _format_set(
    ...
    formatted_items = [
        _format_debug_value(item, renderer, depth + 1, max_depth, max_items) for item in items
    ]
